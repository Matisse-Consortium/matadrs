...
#Initialize closure phase with same shape as input
    ...
    for i in np.arange(4):
        ...
        sin_avg[i,:] = (sign[i,0] * sin_iit3p[idx[i,0],:] + sign[i,1] * sin_oit3p[idx[i,1],:] +\
                          sign[i,2] * sin_iot3p[idx[i,2],:] + sign[i,3] * sin_oot3p[idx[i,3],:])/\
                          (nrepeatii+nrepeatoi+nrepeatio+nrepeatoo)
...
#Treat differential phases
    ...
    for i in np.arange(6):
        ...
        #                 (nrepeatii+nrepeatoi+nrepeatio+nrepeatoo)
        sin_avg[i,:] = (sign[i,0] * sin_iidp[idx[i,0],:] + sign[i,1] * sin_oidp[idx[i,1],:] +\
                        sign[i,2] * sin_iodp[idx[i,2],:] + sign[i,3] * sin_oodp[idx[i,3],:])/\
                        (nrepeatii+nrepeatoi+nrepeatio+nrepeatoo)
...
# Treat visamp (or correlated fluxes)
    ...
    if plot:
        plt.figure(53)
    for i in np.arange(6):
        vafinal[i,:] = (iiva[i,:] + oiva[idx[i,1],:] + iova[idx[i,2],:] + oova[idx[i,3],:])\
                       /(nrepeatii+nrepeatoi+nrepeatio+nrepeatoo)
        if plot:
            plt.subplot(3, 2, i + 1);
