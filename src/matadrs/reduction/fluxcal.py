...
cal_database_paths = [cal_database_dir+'vBoekelDatabase.fits',
...
fluxcal(inputfile_sci, inputfile_cal, outputfile, cal_database_paths,
        mode='corrflux',output_fig_dir=output_fig_dir)
...
cal_database_paths: list of paths to the calibrator databases
(e.g., [caldb1_path,caldb2_path])
mode (optional):
...
output_fig_dir (optional): if it is a valid path, the script will make a plot of the
calibrator model spectrum there, default: '' (= no figure made)
"""
...
# TODO: calculate uncertainties partly implemented caveats: uncertainty in the calibrator
# spectrum is not taken into account uncertainty in calibrator diameter is not taken into
...
# match_radius [arcsec]
# ra, dec [degree]
def get_spectrum_caldb(cal_database_path, cal_name, out_lst,
                       ra=np.nan, dec=np.nan, match_radius=20.0, band='L'):
    """"""
    ...
    if (min_sep < match_radius*u.deg/3600.0):
        ...
        print('Calibrator found in the database '+caldb_file+': '
              +cal_name_lst[min_sep_idx]+ ', separation: %.2f arcsec'
        ...
        print('Closest match: '+cal_name_lst[min_sep_idx]
...
def plot_spec(cal_name, cal_database_path, fig_dir='.', ra=np.nan, dec=np.nan,
    ...
    if match:
        cal_name_db, diam_cal, diam_err_cal, wav_cal,\
...
def fluxcal(inputfile_sci, inputfile_cal, outputfile,
            cal_database_paths, mode='flux', output_fig_dir='',
            match_radius=25.0, do_airmass_correction=False, calc_spectrum_offset=False):
    """"""
    ...
    if match:
        ...
        if math.isnan(diam_cal):
            ...
            if mode != 'flux':
                ...
                os.remove(outputfile)
                return 4
        print('Diameter = %.1f +/- %.1f mas (%s)'
              % (diam_cal, diam_err_cal, cal_name_db))

        if 'calib_spec' in caldb_file:
            wav_cal_model = np.flip(wav_cal_model)
            spectrum_cal = np.flip(spectrum_cal)  # Jy
        wl = np.concatenate((np.array([wav_cal_model[0]
                                       - (wav_cal_model[1] - wav_cal_model[0])]),
                             wav_cal_model))
        wh = np.concatenate((wav_cal_model,
                             np.array([wav_cal_model[-1]
                                       + (wav_cal_model[-1] - wav_cal_model[-2])])))
        wm = (wh + wl) / 2
    ...
    if not math.isnan(spectral_binning) and np.all(np.isfinite(dl_coeffs)):
        ...
        idx = np.logical_and(wav_cal_model > 0.99*np.nanmin(wav_cal),
                             wav_cal_model < 1.01*np.nanmax(wav_cal))
        ...
        spectrum_cal_resampled =\
                transform_spectrum_to_real_spectral_resolution(wav_cal_model*1e6,
                                                               spectrum_cal, dl_coeffs,
                                                               kernel_width_px,
                                                               wav_sci*1e6,
                                                               spectral_binning,
                                                               make_plot=make_plot,
                                                               figpath_out=figpath_out,
                                                               ylabel='Flux (Jy)',
                                                               yscale='final')
    else:
        # old way
        ...
        idx = np.logical_and(wav_cal_model > np.nanmin(wav_cal),
                             wav_cal_model < np.nanmax(wav_cal))
        if 2.0*len(wav_cal_model) < np.nansum(idx):
            ...
            wl = np.concatenate((np.array([wav_cal_model[0] - (wav_cal_model[1]
                                                               - wav_cal_model[0])]),
                                 wav_cal_model))
            wh = np.concatenate((wav_cal_model,np.array([wav_cal_model[-1]
                                                         + (wav_cal_model[-1]
                                                            - wav_cal_model[-2])])))
            wm = (wh+wl)/2
            wav_bin_lower = wm[:-1]
            ...
            for i in range(len(wav_cal_model)):
                ...
                sum = 0.0
                sum = sum + (wav_bin_upper_cal[wi[0]]
                             - wav_bin_lower[i])*spectrum_cal[wi[0]]
                sum = sum + (wav_bin_upper[i]
                             - wav_bin_lower_cal[wi[-1]]) * spectrum_cal[wi[-1]]
    ...
    if os.path.exists(output_fig_dir):
        ...
        if 2.0*len(wav_cal) < np.nansum(idx):
            plt.plot(wav_cal_model*1e6, spectrum_cal, '-',
                     color='grey', label='Original sp.', lw=1.0, alpha=0.66)
        else:
            plt.plot(wav_cal_model*1e6, spectrum_cal, '-o',
                     color='grey', label='Original sp.', lw=1.0, alpha=0.66)
    ...
    if do_airmass_correction:
        ...
        # Transform the transmission to the wavelength grid of the data
        ...
        trans_sci_final = transform_spectrum_to_real_spectral_resolution(wl_um_sci,
                                                                         trans_sci,dl_coeffs,
                                                                         kernel_width_px,
                                                                         wav_sci*1e6,
                                                                         spectral_binning,
        ...
        trans_cal_final = transform_spectrum_to_real_spectral_resolution(wl_um_cal,
                                                                         trans_cal,
                                                                         dl_coeffs,
                                                                         kernel_width_px,
                                                                         wav_sci*1e6,
        ...
        idx = np.logical_and(wav_sci*1e6 > wl_min+wl_range*0.1,
                             wav_sci*1e6 < wl_max-wl_range*0.1)
        ...
        try:
            for j in range(len(inhdul_sci['OI_FLUX'].data['FLUXDATA'])):
                ...
                if do_airmass_correction:
                    if calc_spectrum_offset:
                        ...
                        rp_list_cal.append(calc_corr_offset(trans_cal_final,
                                                            flux_raw_cal,shift_max))
                        rp_list_sci.append(calc_corr_offset(trans_sci_final,
                                                            flux_raw_sci,shift_max))
            ...
            if do_airmass_correction:
                if calc_spectrum_offset:
                    ...
                    plot_corr_offset(rp_list_sci,
                                     np.arange(-shift_max, +shift_max),
                                     outputdir+'/skycalc_correlation_flux_'+tag_sci+'.png')
                    plot_corr_offset(rp_list_cal,
                                     np.arange(-shift_max, +shift_max),
    ...
    if mode == 'corrflux' or mode == 'both':
        ...
        for j in range(len(inhdul_sci['OI_VIS'].data['VISAMP'])):
            ...
            vis_cal = 2*j1(math.pi*diam_cal_rad*spatial_frequency) /\
                    (math.pi*diam_cal_rad*spatial_frequency)
            # plt.figure()
            ...
            if do_airmass_correction:
                if calc_spectrum_offset:
                    ...
                    if len(trans_cal_final) == len(corrflux_raw_cal):
                        rp_list_cal.append(calc_corr_offset(trans_cal_final,
                                                            corrflux_raw_cal,shift_max))
                        rp_list_sci.append(calc_corr_offset(trans_sci_final,
                                                            corrflux_raw_sci,shift_max))
    ...
    if os.path.exists(output_fig_dir):
        ...
        ax1.set_ylim([0.7*np.min(spectrum_cal_resampled),
                      1.1*np.max(spectrum_cal_resampled)])
...
def update_corrflux_from_vis2(inputfile, outputfile, flux_Jy):
    ...
    for j in range(len(outhdul['OI_VIS'].data['VISAMP'])):
        ...
        viserr = np.abs(0.5*inhdul['OI_VIS2'].data['VIS2ERR'][j]/\
                np.sqrt(inhdul['OI_VIS2'].data['VIS2DATA'][j]))
...
def create_skycalc_inputfile(fname, airmass, pwv, wmin, wmax,
                             wgrid_mode='fixed_wavelength_step', wdelta=0.1,
                             wres=20000.0, lsf_type='none', lsf_gauss_fwhm=5.0):
    """"""
...
def transform_spectrum_to_real_spectral_resolution(wl_orig, spec_orig, dl_coeffs,
                                                   kernel_width_px, wl_final,
                                                   spectral_binning, make_plot=False,
