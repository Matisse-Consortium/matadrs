...
# The name of this program.  For info/warning/error messages
THIS_PROG = 'getdata_ESO_archive.py'
VERSION = 1.0
...
TTIME = str('%04i-%02i-%02iT%02i:%02i:%02i' %
            (YEAR, MONTH, DAY, HOUR, MIN, SEC))

# Output results of the initial query
OUT_LIST = 'output_query_' + TTIME + '.' + \
    OUT_FORMAT

# Raw files contained in the initial query
RAW_LIST = 'raw_list_' + TTIME + '.ascii'
SUBMIT_LIST = 'submit_' + TTIME + '.ascii'         # submitted request to archive

# Status of archive request: submitted, error, or completed
STATE_LIST = 'state_' + TTIME + '.ascii'

# Download shell file from ESO archive
DOWN_LIST = 'download_' + TTIME + '.sh'
...
def clean_up(text: str, strip_chars: Optional[List] = [],
             replace_extras: Optional[Dict] = {}) -> str:
    ...
    ----------
    text : str
    strip_chars : list of str, optional
    replace_extras : dict, optional
    ...
    # NOTE: Normalize whitespace and handle replace_extras
...
if NARG < 3:
    print(
        THIS_PROG, '[error] insufficient number of arguments given to', THIS_PROG)
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(
                    THIS_PROG, '[error] -Password <user_password> parameter is missing.')
                print(
                    THIS_PROG, '[error] -User <user_name> parameter is missing.')
                print('USAGE: getdata_ESO_archive.py -User <user_name> -Password <password> -TargetName <SIMBAD target name> -RA <"hh:mm:ss"> -DEC <"dd:mm:ss"> -Box <search box: "hh:mm:ss"> -RunId <run ID> -Inst <instrument name> -StartDate <start date> -EndDate <end date> -Category <file category: SCIENCE, CALIB, ACQUISITION> -Type <file type: OBJECT, STD, BIAS, etc.> -Mode <file mode: IMAGE, SPECTRUM, MOS, IFU, etc.> -MaxRows <max rows to retrieve> -Download <download files: Y/N> -Deliver <delivery_type: files, files_calib, files_master_calib> <optional: -verbose> <optional to create a record of your query: -record>')
                print(' ')
                print(
...
if i_inst != None:
    # instrument name must be capitalised
    inst = sys.argv[i_inst+1].upper()
else:
    inst = ''
...
print(' ')
print(
    THIS_PROG, '[info] will query the ESO archive with the following parameters:')
print(' ')
if verbose == True:
...
if downtype == 'files':
    print('Download Type: %s' %
          ('Only searched-for raw files will be downloaded'))
...
if Nquery == 0:
    ...
    print(THIS_PROG,
          '[info] A total of %i records were found matching the provided criteria.' % (Nquery))
    print(THIS_PROG, '[info] (a maximum limit of %i records were specified in the query ... more may exist)' % (
        int(maxr)))
    print(' ')
...
end_phrase = 'A maximum of %i records were found matching the provided criteria - any remaining rows were ignored.' % (
    Nquery)
clean_output = clean_up(query_output, replace_extras={
                        '#': '', end_phrase: '', '\xef\xbb\xbf': '', 'N/A': 'NA'})
...
regex = re.compile(r'"([a-xA-Z0-9,]+),([a-xA-Z0-9,]+),([a-xA-Z0-9,]+)"')
...
print('%30s %15s %20s %20s' %
      ('FileName', 'Category', 'File_Type', 'File_Mode'))
print('%72s' % (90*'='))
for i in range(Nfiles):
    print('%30s %15s %20s %20s' %
          (files[i], dpr_catg[i], dpr_type[i], dpr_mode[i]))
...
if downtype == 'files':
    postdata = 'requestDescription=script&requestCommand=SELECTIVE_HOTFLY&dataset=%s' % (
        submit_files)

# get the specific data files + raw calibrations and update their headers:
if downtype == 'raw2raw':
    postdata = 'requestDescription=script&requestCommand=SELECTIVE_HOTFLY&requestCommand=CalSelectorRaw2Raw&dataset=%s' % (
        submit_files)

# get the specific data files + master calibrations and update their headers:
if downtype == 'raw2master':
    postdata = 'requestDescription=script&requestCommand=SELECTIVE_HOTFLY&requestCommand=CalSelectorRaw2Master&dataset=%s' % (
        submit_files)
...
newline = 'machine dataportal.eso.org login ' + \
...
try:
    ...
    print(THIS_PROG, '[error] the file %s was not found.  Something is wrong with this archive request.  Exiting!' % (
        SUBMIT_LIST))
    sys.exit()
...
while status != 'COMPLETE':
    ...
    if status == 'ERROR':
        print(
            THIS_PROG, '[error] there is an error with this archive request.  Exiting!')
        sys.exit()
    ...
    if status == '':
        print(
            THIS_PROG, '[error] The state of your submitted archive request is blank!')
        print(
            THIS_PROG, '[error] This probably indicates that either your user name or password is incorrect.')
...
if down == True:
    ...
    print(THIS_PROG, '[info] archive download file can be found in %s/%s' %
          (DOWN_DIR, DOWN_LIST))
...
if record == True:
    ...
    print(
        THIS_PROG, '[info] results of original query saved as %s' % (OUT_LIST))
    print(
        THIS_PROG, '[info] list of requested raw files saved as %s' % (RAW_LIST))
    print(' ')
    print(' ')
