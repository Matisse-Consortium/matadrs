...
def avg_oifits(infiles: List[Path], outfile: Path,
               ...
               filter_lst_flux: Optional[List[int]] = [],
    ...
    avg_baselines: bool, optional
        If True, the function will average all the baselines.
    avg_groups: list, optional
        List of groups to be averaged.
    filter_lst_flux: list of int, optional
    filter flux data to average (experimental), example: [[],[0,1],[2,3]],
      A list for each file: a list of row numbers (starting from 0) to be averaged
    filter_lst_corrflux: list, optional
        List of groups to be averaged.
    scale_corrflux: bool, optional
        If True, the function will scale the corr flux data.
    scale_wl_range: list, optional
        List of wl ranges to be scaled.
    scale_flux: bool, optional
        If True, the function will scale the total flux data.
    ...
    if os.path.exists(infiles[0]):
        ...
        print(f"ERROR (avg_oifits): File not found: {infiles[0]}")
        return
    outhdul = fits.open(outfile, mode="update")
...
def oifits_patchwork(infile_list,outfile_path,oi_types_list=[['vis2','visamp','visphi','t3','flux']],headerval=[]):
    #print(infile_list)
...
def calc_vis_from_corrflux(input_corrflux_file,input_totalflux_file,outfile_path):
    ...
    for k in range(len(outhdul['OI_VIS'].data['VISAMP'])):
        ...
        outhdul['OI_VIS2'].data['VIS2ERR'][k] = vis2err #WARNING: this is not how errors should be calculated
...
def oifits_restrict_wavelengths(infile_path,outfile_path,sel_wl=3.0,bandwidth=1000.0,wl_range=[],do_flag=False):
